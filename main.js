...
// ===================== Map Notes (Client) =====================
// 依赖：Leaflet 已加载，且你有一个全局 map = L.map(...)
// CSS：单独放 notes.css（你说会分开）
// -------------------------------------------------------------

const NOTES_API_BASE = "https://YOUR_WORKER_DOMAIN"; // 例如 https://xxx.workers.dev
const ADMIN_TOKEN = ""; // 可选：填了才启用审核工具按钮（不填也不影响普通用户）

// 刷新前可改：token 只存在内存
// noteId -> { token: string }
const editTokenMem = new Map();
...
function normalizeUrl(u) {
  ...
  if (!s) return "";
  // 允许 http/https；如果用户没写协议，默认 https
  if (/^https?:\/\//i.test(s)) return s;
  return "https://" + s;
}

// -------------------- 创建 DivIcon（圆点） --------------------
function makeNoteIcon(label = "N") {
  return L.divIcon({
    className: "leaflet-note-pin",
    html: `<div class="pin">${esc(label)}</div>`,
    iconSize: [22, 22],
    iconAnchor: [11, 22],
    popupAnchor: [0, -22],
  });
...
// -------------------- 构建弹窗（编辑态 / 展示态） --------------------
function buildPopupHTML(note, mode) {
  // note: {id, title, body, link_text, link_url, status?}
  const title = esc(note.title || "");
  const body = esc(note.body || "");
  const linkText = esc(note.link_text || "");
  const linkUrl = esc(note.link_url || "");

  if (mode === "view") {
    const hasLink = !!note.link_url;
    const linkHtml = hasLink
      ? `<div class="np-links">
           <div class="np-linkrow">
             <span class="np-label">Source:</span>
             <a class="np-a" href="${esc(note.link_url)}" target="_blank" rel="noopener">${esc(note.link_text || note.link_url)}</a>
           </div>
         </div>`
      : "";
    ...
    return `
      <div class="note-popup">
        <div class="np-head">
          <div class="np-title">${title || "Untitled"}</div>
        </div>

        <div class="np-body">
          <div class="np-text">${body ? body.replaceAll("\n", "<br/>") : `<span class="np-muted">No details.</span>`}</div>
          ${linkHtml}
        </div>

        <div class="np-foot">
          <button class="np-btn np-btn-secondary" data-action="edit">Edit (this session)</button>
        </div>
      </div>
    `;
  }
  ...
  return `
    <div class="note-popup">
      <div class="np-head">
        <input class="np-input np-title-input" type="text" placeholder="Title" value="${title}">
      </div>

      <div class="np-body">
        <textarea class="np-textarea" placeholder="Body (Enter saves, Shift+Enter for newline)">${body}</textarea>

        <div class="np-row">
          <input class="np-input" data-field="link_text" type="text" placeholder="Link text (optional)" value="${linkText}">
        </div>
        <div class="np-row">
          <input class="np-input" data-field="link_url" type="text" placeholder="Link URL (optional)" value="${linkUrl}">
        </div>

        <div class="np-hint">
          Enter 保存；Shift+Enter 换行。点地图其他地方也会保存并退出编辑。
        </div>
      </div>

      <div class="np-foot">
        <button class="np-btn np-btn-primary" data-action="save">Save</button>
        <button class="np-btn np-btn-danger" data-action="cancel">Cancel</button>
      </div>
    </div>
  `;
}
...
// -------------------- 绑定弹窗里的按钮/输入事件 --------------------
function bindPopupHandlers(marker, noteId) {
  const el = marker.getPopup()?.getElement();
  if (!el) return;

  const btnEdit = el.querySelector('[data-action="edit"]');
  const btnSave = el.querySelector('[data-action="save"]');
  ...
  if (btnSave) {
    btnSave.addEventListener("click", () => {
      commitEdit(noteId, { closeAfter: true });
    });
  }

  if (btnCancel) {
    btnCancel.addEventListener("click", () => {
      exitEditMode(noteId, { revert: true });
    });
  }

  // Enter 保存：textarea 里 Shift+Enter 才换行
  const ta = el.querySelector(".np-textarea");
  if (ta) {
  ...
  // 标题输入 Enter 也保存
  const titleInput = el.querySelector(".np-title-input");
  if (titleInput) {
...
function enterEditMode(noteId) {
  const pack = noteMarkers.get(noteId);
  if (!pack) return;

  // 只允许本 session 创建的点编辑：必须有 token
  const tokenPack = editTokenMem.get(noteId);
  if (!tokenPack?.token) {
    // 没 token 就别让他进编辑（刷新后/别人的点）
    alert("This note cannot be edited after refresh.");
    return;
  }

  _editingNoteId = noteId;

  const marker = pack.marker;
  const note = pack.data;

  marker.setPopupContent(buildPopupHTML(note, "edit"));
  marker.openPopup();

  // 绑定输入/按钮
  marker.once("popupopen", () => bindPopupHandlers(marker, noteId));

  // focus
  setTimeout(() => {
    const el = marker.getPopup()?.getElement();
    const ti = el?.querySelector(".np-title-input");
    if (ti) ti.focus();
  }, 0);
}

// -------------------- 退出编辑模式 --------------------
function exitEditMode(noteId, { revert = false } = {}) {
  const pack = noteMarkers.get(noteId);
  if (!pack) return;

  const marker = pack.marker;
  if (revert && pack._snapshot) {
    pack.data = { ...pack._snapshot };
    delete pack._snapshot;
  }

  _editingNoteId = null;

  marker.setPopupContent(buildPopupHTML(pack.data, "view"));
  marker.openPopup();
  marker.once("popupopen", () => bindPopupHandlers(marker, noteId));
}

// -------------------- 读取编辑表单 --------------------
function readEditForm(marker) {
  const el = marker.getPopup()?.getElement();
  if (!el) return null;

  const title = el.querySelector(".np-title-input")?.value ?? "";
  const body = el.querySelector(".np-textarea")?.value ?? "";
  const link_text = el.querySelector('[data-field="link_text"]')?.value ?? "";
  const link_url_raw = el.querySelector('[data-field="link_url"]')?.value ?? "";

  const link_url = normalizeUrl(link_url_raw);
...
// -------------------- 提交编辑到 Worker --------------------
async function commitEdit(noteId, { closeAfter = true } = {}) {
  const pack = noteMarkers.get(noteId);
  if (!pack) return;

  const token = editTokenMem.get(noteId)?.token;
  if (!token) {
    alert("This note cannot be edited after refresh.");
    return;
  }

  const form = readEditForm(pack.marker);
  if (!form) return;
  ...
  // 先保存 snapshot（取消时回滚）
  pack._snapshot = { ...pack.data };

  try {
    const res = await fetch(`${NOTES_API_BASE}/api/notes/${noteId}`, {
      method: "PATCH",
      headers: {
    ...
    // 本地更新
    pack.data = { ...pack.data, ...form };
    delete pack._snapshot;

    // 切回 view
    if (closeAfter) {
      exitEditMode(noteId, { revert: false });
...
// -------------------- 双击地图：创建新 note（立即进入编辑） --------------------
async function createNoteAt(latlng) {
  // 先在地图上放一个临时 marker（让用户立即看到）
  ...
  const marker = L.marker(latlng, { icon: makeNoteIcon("N") }).addTo(map);
  marker.bindPopup(buildPopupHTML(tempData, "edit"), { className: "leaflet-note-popup", maxWidth: 560 });

  noteMarkers.set(tempId, { marker, data: tempData });
  _editingNoteId = tempId;

  marker.openPopup();
  marker.once("popupopen", () => bindPopupHandlers(marker, tempId));

  // 发 POST 创建
  try {
    const res = await fetch(`${NOTES_API_BASE}/api/notes`, {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        body: tempData.body,
        link_text: tempData.link_text,
        link_url: tempData.link_url,
      }),
    });
    ...
    // 用真实 id 替换 tempId
    const realId = out.id;
    const edit_token = out.edit_token;

    // 迁移 map
    const pack = noteMarkers.get(tempId);
    noteMarkers.delete(tempId);

    pack.data.id = realId;
    pack.data.lat = latlng.lat;
    pack.data.lng = latlng.lng;

    noteMarkers.set(realId, pack);
    editTokenMem.set(realId, { token: edit_token });

    // 重新绑定 popup handler 用新 id
    pack.marker.setPopupContent(buildPopupHTML(pack.data, "edit"));
    pack.marker.openPopup();
    pack.marker.once("popupopen", () => bindPopupHandlers(pack.marker, realId));

    // 更新 editingNoteId
    _editingNoteId = realId;

    // focus title
    setTimeout(() => {
      const el = pack.marker.getPopup()?.getElement();
      const ti = el?.querySelector(".np-title-input");
      if (ti) ti.select();
    }, 0);
  } catch (e) {
...
async function loadApprovedNotes() {
  try {
    const res = await fetch(`${NOTES_API_BASE}/api/notes`, { method: "GET" });
    const list = await res.json().catch(() => []);
    if (!Array.isArray(list)) return;

    // 先清掉旧的 approved（不动 session 新增的）
    for (const [id, pack] of noteMarkers.entries()) {
      if (!id.startsWith("temp-") && !editTokenMem.has(id) && pack.data?._source === "approved") {
        map.removeLayer(pack.marker);
    ...
    for (const n of list) {
      if (!n?.id) continue;
      if (noteMarkers.has(n.id)) continue; // 本 session 新建的别覆盖
      ...
      const marker = L.marker([n.lat, n.lng], { icon: makeNoteIcon("N") }).addTo(map);
      marker.bindPopup(buildPopupHTML(data, "view"), { className: "leaflet-note-popup", maxWidth: 560 });

      noteMarkers.set(n.id, { marker, data });
      marker.on("popupopen", () => bindPopupHandlers(marker, n.id));
    }
  } catch (e) {
    console.error(e);
  }
...
// -------------------- 点击地图其他地方：若正在编辑，则保存并退出 --------------------
function installClickOutsideCommit() {
  map.on("click", () => {
    if (!_editingNoteId) return;
    const id = _editingNoteId;

    // 只有有 token 的才会 commit（也就是本 session 新建的）
    if (!editTokenMem.get(id)?.token) {
      _editingNoteId = null;
      return;
    }
    commitEdit(id, { closeAfter: true });
  });
}

// -------------------- 双击事件：创建 note --------------------
function installDoubleClickCreate() {
  // Leaflet 默认双击是 zoom，需要关掉
...
function installAdminTools() {
  ...
  const btn = document.createElement("button");
  btn.className = "notes-admin-btn";
  btn.textContent = "Admin Pending";
  btn.onclick = async () => {
    const res = await fetch(`${NOTES_API_BASE}/api/admin/pending`, {
    ...
    if (!Array.isArray(items) || items.length === 0) {
      alert("No pending notes.");
      return;
    }

    const pick = prompt(
      `Pending IDs:\n${items.slice(0, 12).map(x => x.id).join("\n")}\n\nInput an ID to approve:`
    );
    if (!pick) return;

    await fetch(`${NOTES_API_BASE}/api/admin/approve`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Admin-Token": ADMIN_TOKEN,
      },
      body: JSON.stringify({ id: pick.trim() }),
    });

    alert("Approved (if ID existed). Refresh approved layer.");
    loadApprovedNotes();
  };
...
// -------------------- 初始化入口 --------------------
export function initMapNotes() {
  installDoubleClickCreate();
  installClickOutsideCommit();
  loadApprovedNotes();
  installAdminTools();

  // 你也可以定时刷新 approved
  // setInterval(loadApprovedNotes, 15000);
}
